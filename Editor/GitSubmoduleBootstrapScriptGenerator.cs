    ...
    internal static class GitSubmoduleBootstrapScriptGenerator
    {
        ...
        internal static string BuildScriptContent()
        {
            string[] lines =
            {
                ...
                ")",
                "if exist \"%INDEX_LOCK%\" (",
                "    echo [Git Submodules] ERROR: A Git index lock exists.",
                "    echo Wait for clone / checkout / Git LFS to finish, then run this script again.",
                "    echo If no Git process is running, inspect the stale lock before removing it.",
                "    goto :fail",
                ")",
                "",
