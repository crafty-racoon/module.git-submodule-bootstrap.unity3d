    ...
    internal static class GitSubmoduleBootstrapScriptGenerator
    {
        ...
        internal static string BuildScriptContent(string unityProjectRelativePath)
        {
            ...
            string[] lines =
            {
                ...
                "    )",
                "",
                "    powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command \"$p = Join-Path (Get-Location) '" + unityProjectLockPath + "'; try { $s = [System.IO.File]::Open($p, [System.IO.FileMode]::Open, [System.IO.FileAccess]::ReadWrite, [System.IO.FileShare]::None); $s.Dispose(); exit 0 } catch { exit 1 }\"",
                "    if errorlevel 1 (",
                ...
                "    echo Resolve the submodule state manually or use Unity Tools ^> Git Submodules ^> Update Now.",
                "    goto :fail",
                ")",
                "",
